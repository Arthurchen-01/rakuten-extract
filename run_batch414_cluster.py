# -*- coding: utf-8 -*-
"""
Batch 414 Cluster Orchestrator
Dispatches 414 unique accounts across 6 nodes with 3 threads each (18 threads total).
...
ACCOUNTS_SOURCE = BASE_DIR / "data" / "batch_414_accounts.json"
RUN_DIR = BASE_DIR / "data" / "local_deep_audit" / "batch414_run"
...
def deploy_and_init():
    ...
    def _deploy(node):
        ...
        ssh = get_ssh(node)
        ssh.exec_command(f"mkdir -p {node['run_dir']}/batch414_run/screenshots")
...
def launch_batch_run():
    ...
    per_node = (total + len(NODES) - 1) // len(NODES) # 69
    
    print("\n" + "=" * 85)
    print(f"📦 [任务切片分发] 总计 {total} 户账号 | 分发至 6 节点 | 每节点 {per_node} 户 | 每机 3 线程并发 (全网 18 线程)")
    print("=" * 85)
    
    slices = {}
    for i, node in enumerate(NODES):
        nid = node['id']
        s_accs = all_accounts[i * per_node : (i + 1) * per_node]
        slices[nid] = s_accs
    ...
    def _start_node(node):
        ...
        remote_accs_file = f"{node['run_dir']}/accounts_batch414.json"
        remote_out_dir = f"{node['run_dir']}/batch414_run"
...
def monitor_and_collect():
    ...
    while True:
        ...
        def _check_node(node):
            ...
            try:
                ssh = get_ssh(node)
                stdin, stdout, stderr = ssh.exec_command(f"cat {node['run_dir']}/batch414_run/batch_progress.json 2>/dev/null")
                ...
                if content:
                    ...
                    return nid, prog
                return nid, {'is_running': p_cnt > 0, 'processed_count': 0, 'total_accounts': 69, 'active_count': 0, 'process_count': p_cnt}
            except Exception as e:
                return nid, {'is_running': False, 'error': str(e), 'processed_count': 0, 'total_accounts': 69, 'active_count': 0}
        ...
        print(f"  📊 全网大盘汇总: 累计推进 {total_processed}/414 ({total_processed/414*100:.1f}%) | 确认活跃大号: {total_active} | 提取真实订单: {total_orders}")
        
        if all_completed and total_processed >= 414:
            print("\n🎉 全网 6 节点全部 414 户账号执行完毕！")
            break
        elif elapsed > 3600:
    ...
    excel_name = f"9.8晚上_414户全量汇总_{active_cnt}活跃-{failed_cnt}失败.xlsx"
    ...
    p1 = RUN_DIR / "batch414_excel_sheet1_preview.png"
    p4 = RUN_DIR / "batch414_excel_sheet4_preview.png"
    
    render_sheet_preview_image(ws1, p1, f"乐天导出导入主表 (Sheet 1) - 414 户全集群 18 线程实测原件视图 (共 {ws1.max_row - 1} 户)", max_r=18, max_c=16)
    render_sheet_preview_image(ws4, p4, f"乐天订单明细总览 (Sheet 4) - 414 户全量订单原件视图 (共 {ws4.max_row - 1} 笔)", max_r=20, max_c=8)
    
    shutil.copy(p1, ARTIFACT_DIR / p1.name)
    shutil.copy(p4, ARTIFACT_DIR / p4.name)
    
    print("=" * 85)
    print(f"🎉 414 户全量实测与法定真本导出圆满完成！活跃: {active_cnt} | 失败: {failed_cnt}")
    print("=" * 85)
